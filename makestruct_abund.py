...
# Now we wanted to check for the existance of the metalicity keyword, but we import it directly so how can
# Same for non-existence of alpha
# we test if it exists or not? when doesn't it? when does it? @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
# Interpolates for the point on this graph for metalicity. Not sure if we need to change these values ever? @@@@@@@@@@@
# Swapped order from IDL as it goes y, x and we want x, y
interpolation_line = interp1d([-10.,-1.,0.,10.], [0.4,0.4,0.0,0.0], kind="linear")
# Check values with SVen. Basically finds the data for the metalicity data point. (Just 1? ) @@@@@@@@@@@
alpha_enhancement = interpolation_line(int(metalicity_modified))
