...
def structure_creation(segment_mask):
    ...
    #End of make_obs function, beginning of pre_norm.. hmm..
    ...
    linemask_data = pd.read_csv(
        "DATA/" + line_mask, sep="[ ]{2,}", header=None, engine='python',
    ...
    if run == 2:
        ...
        for line in range(0, len(linemask_data['Sim_Wavelength_Peak'])):
            ...
            inside_linemask_index = np.where(np.logical_and(np.logical_and(line_atomic[:,2] > linemask_data['Sim_Wavelength_Start'][line]-buffer,
                                        line_atomic[:,2] < linemask_data['Sim_Wavelength_End'][line]+buffer), depth > galah_sp_part1.depthmin))
            xx.extend(inside_linemask_index[0])
            ...
            if peak_index.size != 0:
                ...
                print("No peak line (",linemask_data['Sim_Wavelength_Peak'][line],  ") available in the atomic line list for line",
                      line, "(", linemask_data['Sim_Wavelength_Start'][line],"to", linemask_data['Sim_Wavelength_End'][line],")")
    ...
    try:
        galah_sp_part1.atmosphere_grid_file
