...
#from age_mass_guess_parsec import read_iso
# In[ ]:
...
def optimize_logg_with_extra_input(
    current_logg, teff, fe_h, field, glob_free, 
    elli_dir=r'C:\Users\jama2357\Documents\Galafiles\GALAH\ELLI',
    ks_m=None, ks_msigcom=None, d=None, d_lo=None, d_hi=None, a_ks=None, e_a_ks=None, ebv=None,
    nu_max=None,
    ...
    2) estimated from extra asteroseismic data (e.g. nu_max)
        This is the case, when '_seis' is part of the 'field' name
    3) estimated from extra data like astrometric and photometric data (e.g. ks_m == 2MASS K_S apparent magnitude)
        This is the case, when '_lbol' is part of the 'field' name.
        ...
        LBOL we get from the astrometric+photometric information and an estimate of the bolometric correction (KS_M, D, A_KS, BC_KS)      
    ...
    4) kept fixed 
        ...
        and neither _lbol or _seis are part of the 'field' name.
    ...
    ks_m         : 2MASS Ks band magnitude
    ks_msigcom   : Uncertainty 2MASS Ks band magnitude
    d            : Best distance estimate [pc]
    d_lo         : Low percentile distance estimate [pc]
    d_hi         : High percentile distance estimate [pc]
    ...
    try:
        ...
        y = np.load(r'C:\Users\jama2357\Documents\Galafiles\GALAH\ELLI\Parsec_isochrones.npy',allow_pickle=True,encoding='latin1')
    ...
    def lbol_function(ks_m, ks_msigcom, d, d_lo, d_hi, a_ks, e_a_ks, bc_ks, e_bc_ks = 0.05, m_bol_sun = m_bol_sun, debug=False, show_todo=True):
        """
        This function computes the bolometric luminosity from the Ks magnitude
        
        INPUT:
        ks_m       : 2MASS magnitude in Ks band
        ks_msigcom : 
    ...
    def update_logg_parallax(teff, current_logg, fe_h, ks_m, ks_msigcom, d, d_lo, d_hi, a_ks, e_a_ks, ebv, debug=debug):
        """
        ...
        l_bol, e_lbol = lbol_function(ks_m, ks_msigcom, d, d_lo, d_hi, a_ks, e_a_ks, bc_ks, debug=debug, show_todo=show_todo)
    ...
    if 'GRAV' in glob_free:
        # Case 1
        if debug:
            print('LOGG/GRAV free, return input value')
        ...
        if field[-4:]=='seis':
            ...
            if isinstance(nu_max, float):
                if nu_max>=0.:
                    return(logg_numax(teff, nu_max, debug=debug))
                else:
            ...
            input_values = np.array([ks_m, ks_msigcom, d, d_lo, d_hi, a_ks, e_a_ks, ebv])
            input_names  = np.array(['ks_m', 'ks_msigcom', 'd', 'd_lo', 'd_hi', 'a_ks', 'e_a_ks', 'ebv'])
            ...
            if len(check_if_float)!=0:
                ...
                if len(check_if_finite)!=0:
                    ...
                    updated_logg = update_logg_parallax(teff, current_logg, fe_h, ks_m, ks_msigcom, d, d_lo, d_hi, a_ks, e_a_ks, ebv, debug=debug)
                    if debug:
                        it = 1
...
optimize_logg_with_extra_input(
    current_logg, teff, fe_h, field, glob_free, 
    ks_m=ks_m, ks_msigcom=ks_msigcom, d=d, d_lo=d_lo, d_hi=d_hi, a_ks=a_ks, e_a_ks=e_a_ks, ebv=ebv,
    nu_max=nu_max,
